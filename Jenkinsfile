pipeline {
    ...
    stages {
        ...
        stage("Git Checkout") {
            steps {
                git branch: 'main', url: 'https://github.com/harishnshetty/netflix.git'
            }
        }
        ...
        stage("Build Docker Image") {
            steps {
                script {
                    env.IMAGE_TAG = "harishnshetty/netflix:${BUILD_NUMBER}"

                    // Optional cleanup
                    sh "docker rmi -f netflix ${env.IMAGE_TAG} || true"
            ...
            steps {
                script {
                    withCredentials([string(credentialsId: 'docker-cred', variable: 'dockerpwd')]) {
                        ...
                        sh "docker tag netflix ${env.IMAGE_TAG}"
                        sh "docker push ${env.IMAGE_TAG}"

                        // Also push latest
                        sh "docker tag netflix harishnshetty/netflix:latest"
                        sh "docker push harishnshetty/netflix:latest"
                    }
                }
            }
        }
        ...
        stage("Deploy to Container") {
            steps {
                script {
                    ...
                    sh "docker run -d --name netflix -p 80:80 ${env.IMAGE_TAG}"
                }
            }
        }
    }
